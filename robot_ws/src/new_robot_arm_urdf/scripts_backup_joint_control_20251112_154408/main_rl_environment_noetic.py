...
# VALIDATED WORKSPACE BOUNDARIES (from workspace_analyzer_constrained.py)
# These boundaries are validated against joint limits with IK solver
# Full analysis: Y [-10.60, +10.70] cm, Z [3.00, 19.70] cm, Area: 293.49 cm²
VALIDATED_WORKSPACE = {
    'ultra_conservative': {  # 99% success rate
        'y_min': -0.0742, 'y_max': 0.0749,  # ±7.4cm
        'z_min': 0.07, 'z_max': 0.177       # 7-17.7cm (10.7cm range)
    },
    'conservative': {  # 95% success rate (RECOMMENDED for RL)
        'y_min': -0.0901, 'y_max': 0.0909,  # ±9cm
        'z_min': 0.05, 'z_max': 0.187       # 5-18.7cm (13.7cm range)
    },
    'moderate': {  # 90% success rate
        'y_min': -0.1007, 'y_max': 0.1016,  # ±10cm
        'z_min': 0.04, 'z_max': 0.197       # 4-19.7cm (15.7cm range)
    },
    'full': {  # 100% of validated workspace (some edge failures possible)
        'y_min': -0.106, 'y_max': 0.107,    # ±10.6cm
        'z_min': 0.03, 'z_max': 0.197       # 3-19.7cm (16.7cm range)
    }
}


def is_within_workspace(y, z, safety_level='conservative'):
    """
    Check if (Y, Z) position is within validated workspace boundaries
    
    These boundaries were computed by workspace_analyzer_constrained.py which:
    - Tests 120,701 points on a 1mm resolution grid
    - Uses constrained IK with joint limit enforcement
    - Validates each solution with forward kinematics
    - Ensures robot stays within ±85° for joints 1-3 and 5°-175° for joint 4
    
    Args:
        y: Y position in meters
        z: Z position in meters  
        safety_level: 'ultra_conservative' | 'conservative' | 'moderate' | 'full'
                     Default 'conservative' recommended for RL (95% success)
        
    Returns:
        bool: True if position is within validated workspace
    """
    bounds = VALIDATED_WORKSPACE[safety_level]
...
class RLEnvironmentNoetic:
    ...
    def __init__(self, max_episode_steps=200, goal_tolerance=0.01, enable_gazebo_trajectory=False):
        """
...
```
    ...
    def execute_action(self, action: np.ndarray) -> bool:
        """
        Execute action on the 4DOF robot with VELOCITY LIMITING to prevent Gazebo NaN errors.
        
        CRITICAL FIX: Scales trajectory time based on joint angular distance to prevent
        velocities exceeding 3 rad/s, which causes Gazebo physics to break (NaN joint states).
        ...
        # CRITICAL: Calculate required time to prevent high velocities
        # Get current joint positions
        current_joints = np.array(self.joint_positions) if len(self.joint_positions) == 4 else np.zeros(4)
        
        # Calculate maximum joint angular distance
        joint_distances = np.abs(action_clipped - current_joints)
        max_joint_distance = np.max(joint_distances)
        
        # Velocity limit: 3.0 rad/s (safe limit to prevent Gazebo NaN errors)
        # Higher velocities (>5 rad/s) cause Gazebo physics to break
        MAX_SAFE_VELOCITY = 3.0  # rad/s
        MIN_TIME = 0.5  # Minimum time for very small movements
        
        # Calculate required time: distance / velocity + safety buffer
        required_time = max(max_joint_distance / MAX_SAFE_VELOCITY, MIN_TIME)
        required_time = min(required_time, 5.0)  # Cap at 5 seconds
        
        # Log velocity information
        estimated_velocity = max_joint_distance / required_time if required_time > 0 else 0
        rospy.loginfo(f"   Max joint move: {np.degrees(max_joint_distance):.1f}° → {required_time:.2f}s ({estimated_velocity:.2f} rad/s)")
        ...
        point.time_from_start = rospy.Duration(required_time)
        ...
        try:
            ...
            self.trajectory_action_client.send_goal_and_wait(goal, rospy.Duration(required_time + 3.0))
            ...
            reached = wait_until_reached(self, action_clipped, tol=0.1, timeout=required_time + 5, vel_thresh=0.01)
    ...
    def move_to_joint_positions(self, joint_positions: np.ndarray) -> dict:
        ...
        try:
            ...
            # Surface is at X=0.15m, allow 1cm tolerance for numerical precision
            OVERREACH_LIMIT = 0.16  # 16cm (1cm beyond surface)
            if predicted_x > OVERREACH_LIMIT:
                rospy.logwarn(f"⚠️ OVERREACH WARNING: EE would be at x={predicted_x:.3f}m (> {OVERREACH_LIMIT}m limit)")
    ...
    def _reset_target_position(self) -> bool:
        """Move target sphere to random position on the drawing surface (VALIDATED workspace)"""
        # Drawing surface is a vertical plane at x=0.15m (15cm from robot base)
        # Using VALIDATED workspace boundaries from workspace_analyzer_constrained.py
        ...
        # - Old Z range: 6cm height (12-18cm)
        # - New Z range: 13.7cm height (5-18.7cm) 
        # - Improvement: 2.3x MORE vertical training space! 🎯
        
        # Randomize target position within validated workspace
        sphere_x = surface_x
        sphere_y = random.uniform(surface_y_min, surface_y_max)
        sphere_z = random.uniform(surface_z_min, surface_z_max)
        
        # Safety check: Validate against workspace boundaries (should always pass)
        if not is_within_workspace(sphere_y, sphere_z, safety_level='conservative'):
            rospy.logwarn(f"⚠️ Target ({sphere_y*100:.1f}cm, {sphere_z*100:.1f}cm) outside validated workspace!")
            rospy.logwarn("   Falling back to safe center position...")
            sphere_y = 0.0
